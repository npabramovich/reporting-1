...
--
-- MULTI-VEHICLE: every row is scoped by (fund_id, portfolio_group). fund_id is
-- the company/installation; portfolio_group is the vehicle (an SPV, Fund I, …),
-- matching lp_investments / fund_cash_flows / fund_group_config. Each vehicle has
-- its own separate set of books.
--
-- Invariant: every journal_entry's postings sum to zero per currency
-- (debits positive, credits negative). Enforced in lib/accounting.

-- ---------------------------------------------------------------------------
-- chart_of_accounts — a vehicle's accounts. Per-LP capital accounts carry an
...
create index chart_of_accounts_fund_idx on public.chart_of_accounts (fund_id, portfolio_group, code);
create index chart_of_accounts_lp_entity_idx on public.chart_of_accounts (lp_entity_id) where lp_entity_id is not null;

-- ---------------------------------------------------------------------------
-- fiscal_periods — reporting periods per vehicle; locked once closed.
-- ---------------------------------------------------------------------------
...
create index fiscal_periods_fund_idx on public.fiscal_periods (fund_id, portfolio_group, period_end desc);
...
create index journal_entries_fund_idx on public.journal_entries (fund_id, portfolio_group, entry_date desc);
create index journal_entries_period_idx on public.journal_entries (period_id) where period_id is not null;
create index journal_entries_status_idx on public.journal_entries (fund_id, portfolio_group, status);
...
create index journal_postings_fund_lp_idx on public.journal_postings (fund_id, portfolio_group, lp_entity_id);

-- ---------------------------------------------------------------------------
-- allocation_runs / allocation_results — output of the allocation engine for a
-- period (calls/distributions/fees/expenses/gains → per-LP).
-- ---------------------------------------------------------------------------
...
create index allocation_runs_fund_idx on public.allocation_runs (fund_id, portfolio_group, created_at desc);
...
create index allocation_results_fund_lp_idx on public.allocation_results (fund_id, portfolio_group, lp_entity_id);
